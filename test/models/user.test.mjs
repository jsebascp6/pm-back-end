...
describe('User Model', function() {
  it('should hash the password before saving a user', async function() {
    const user = await User.create({ username: 'testuser', email: 'test@example.com', password: 'plaintextpassword' });
  ...
  it('should validate a correct password', async function() {
    const user = await User.create({ username: 'testuser2', email: 'test2@example.com', password: 'mysecurepassword' });
  ...
  it('should not create a user with an invalid email', function() {
    return expect(User.create({ username: 'invaliduser', email: 'invalidemail', password: 'password123' }))
